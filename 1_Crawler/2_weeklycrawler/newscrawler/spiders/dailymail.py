...
class DailyMailSpider(CrawlSpider):
    ...
    def __init__(self, yearmonth='', *args, **kwargs):
        ...
        begin_date = pd.Timestamp(yearmonth + "-01") - pd.DateOffset(days=1) # Don't change
        end_date   = pd.Timestamp(begin_date) + pd.DateOffset(months=0) - pd.DateOffset(days=0) # Dont change
